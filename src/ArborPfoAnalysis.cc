/**
 *  @file   PandoraAnalysis/src/ArborPfoAnalysis.cc
 * 
...
#include "ArborPfoAnalysis.h"

#include <cmath>
#include <cstdlib>

ArborPfoAnalysis pfoAnalysis;

//------------------------------------------------------------------------------------------------------------------------------------------

ArborPfoAnalysis::ArborPfoAnalysis() :
    Processor("ArborPfoAnalysis"),
    m_nRun(0),
    m_nEvt(0),
    m_nRunSum(0),
    m_nEvtSum(0),
...
{
    _description = "ArborPfoAnalysis analyses output of ArborPFA";
...
void ArborPfoAnalysis::init()
{
    ...
    m_pTTree = new TTree("ArborPfoAnalysisTree", "ArborPfoAnalysisTree");
    ...
    m_pTTree->Branch("m_pfoEnergyTracksClusters", &m_pfoEnergyTracksClusters, "m_pfoEnergyTracksClusters/F");
...
void ArborPfoAnalysis::processRunHeader(lcio::LCRunHeader */*pLCRunHeader*/)
{
    m_nRun = 0;
    m_nEvt = 0;
    ++m_nRunSum;
...
void ArborPfoAnalysis::processEvent(EVENT::LCEvent *pLCEvent)
{
...
void ArborPfoAnalysis::check(EVENT::LCEvent */*pLCEvent*/)
{
}

//------------------------------------------------------------------------------------------------------------------------------------------

void ArborPfoAnalysis::end()
{
    if (m_printing > -1)
    {
        std::cout << "ArborPfoAnalysis::end() " << this->name() << " processed " << m_nEvtSum << " events in " << m_nRunSum << " runs " << std::endl
...
void ArborPfoAnalysis::ExtractCollections(EVENT::LCEvent *pLCEvent)
{
    ...
    std::sort(m_pfoTargetVector.begin(), m_pfoTargetVector.end(), ArborPfoAnalysis::SortPfoTargetsByEnergy);
}

//------------------------------------------------------------------------------------------------------------------------------------------

void ArborPfoAnalysis::ApplyPfoSelectionRules(const EVENT::MCParticle *pMCParticle, MCParticleList &mcPfoCandidates) const
{
...
void ArborPfoAnalysis::MakeQuarkVariables(EVENT::LCEvent *pLCEvent)
{
...
void ArborPfoAnalysis::PerformArborPfoAnalysis()
{
    ...
    for (ParticleVector::const_iterator iter = m_pfoVector.begin(), iterEnd = m_pfoVector.end(); iter != iterEnd; ++iter)
    {
        ...
        if (!pPfo->getTracks().empty())
        {
            ...
            {
                const EVENT::Cluster* cluster = *iter;
				clusterEnergySum += cluster->getEnergy();
			}

			m_pfoEnergyTracksClusters += clusterEnergySum;
        }
        else
        {
            // Neutral pfos
...
bool ArborPfoAnalysis::SortPfoTargetsByEnergy(const EVENT::MCParticle *const pLhs, const EVENT::MCParticle *const pRhs)
{
