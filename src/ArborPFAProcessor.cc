/**
 *  @file   MarlinArbor/src/ArborPFAProcessor.cc
 * 
...
ArborPFAProcessor::ArborPFAProcessor() :
    ...
    m_pPfoCreator(NULL)
{
    _description = "Arbor reconstructs clusters and particle flow objects";
...
const pandora::Pandora *ArborPFAProcessor::GetPandora() const
{
    if (NULL == m_pPandora)
...
const EVENT::LCEvent *ArborPFAProcessor::GetCurrentEvent(const pandora::Pandora *const pPandora)
{
...
{
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, ArborContent::RegisterAlgorithms(*m_pPandora));

    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, ArborContent::RegisterArborPseudoLayerPlugin(*m_pPandora));
...
ArborPFAProcessor::Settings::Settings() :
    m_innerBField(3.5f),
