        ...
        iter != iterEnd; ++iter)
    {
        try
        {
            ...
            for (unsigned int i = 0; i < mcPfoVector.size(); ++i)
            {
                ...
                const float mom[3] = {static_cast<float>(pMCParticle->getMomentum()[0]), 
					                  static_cast<float>(pMCParticle->getMomentum()[1]), 
									  static_cast<float>(pMCParticle->getMomentum()[2])};
            ...
            for (unsigned int i = 0; i < mcQuarks.size(); ++i)
            {
                ...
                const float mom[3] = {static_cast<float>(pQuark->getMomentum()[0]), 
									  static_cast<float>(pQuark->getMomentum()[1]), 
									  static_cast<float>(pQuark->getMomentum()[2])};
                const float mass(std::sqrt(std::max(0.f, energy * energy - mom[0] * mom[0] - mom[1] * mom[1] - mom[2] * mom[2])));
